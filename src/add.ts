...
export async function runAdd(args: string[], options: AddOptions = {}): Promise<void> {
  ...
  if (!source) {
    ...
    console.log(
      `    ${pc.cyan('npx skilldirs add')} ${pc.yellow('<source>')} ${pc.dim('[options]')}`
    );
    console.log();
    console.log(pc.dim('  Example:'));
    console.log(`    ${pc.cyan('npx skilldirs add')} ${pc.yellow('vercel-labs/agent-skills')}`);
    console.log();
    process.exit(1);
  }
  ...
  try {
    ...
    if (sourceOwner === 'openclaw' && !options.dangerouslyAcceptOpenclawRisks) {
      ...
      console.log();
      p.log.message(
        `If you understand the risks, re-run with:\n\n  ${pc.cyan(`npx skilldirs add ${source} --dangerously-accept-openclaw-risks`)}\n`
      );
  ...
  try {
    ...
    if (install) {
      ...
      try {
        ...
        p.log.message(pc.dim('  npx skilldirs add vercel-labs/skills@find-skills -g -y --all'));
      }
    } else {
      // User declined - dismiss the prompt
      await dismissPrompt('findSkillsPrompt');
      p.log.message(
        pc.dim('You can install it later with: npx skilldirs add vercel-labs/skills@find-skills')
      );
    }
  } catch {
